...
class AnimagineXLProvider(ImageProvider):
    def __init__(
        self,
        config: dict[str, Any],
    ) -> None:
        ...
        if (
            ...
            raise RuntimeError(
                "Animagine XL 3.1 is configured for CUDA, "
        ...
        if configured_dtype == "float16":
            ...
            raise RuntimeError(
                f"Unsupported Animagine XL dtype: "
                f"{configured_dtype}"
            )
    ...
    def load(self) -> None:
        ...
        if not self.model_path.exists():
            raise RuntimeError(
                "Animagine XL 3.1 model was not found at "
                f"{self.model_path}\n\n"
    ...
    def generate(
        ...
        if not 512 <= request.height <= 1536:
            raise ValueError(
                "Height must be between "
                "512 and 1536."
            )

        if not 1 <= request.steps <= 40:
            raise ValueError(
                "Animagine XL steps must be "
                "between 1 and 40."
            )
