...
function getConversationStorageKey(
  ...
  return `${CHAT_STORAGE_PREFIX}:${String(
    projectId,
  )}:${sceneId}`
}
...
function loadConversation(
  ...
  try {
    ...
    return {
      ...
      /*
       * Old saved "character" values remain
       * valid because character creation is still
       * supported inside AI Chat.
       */
      chatMode:
        parsed.chatMode ===
        "character"
...
${
  activeScene.time
    ? `Time: ${activeScene.time}`
    : ""
}
${
  activeScene.synopsis
    ? `Synopsis: ${activeScene.synopsis}`
    : ""
}
...
${
  sceneText ||
  "(The current scene does not contain any text yet.)"
}`
}

function buildContextPrompt(
  project: Parameters<
    typeof buildAIContext
  >[0],
  activeScene: Parameters<
    typeof buildAIContext
  >[1],
...
function buildCharacterSystemPrompt(
  ...
  activeScene: Parameters<
    typeof buildAIContext
  >[1],
  currentDraft?: CharacterDraft | null,
): string {
  const context =
    buildAIContext(
      project,
...
The author is designing ONE new fictional character through conversation.

IMPORTANT:
- Do not create the character in the database.
- The application handles creation only after explicit approval.
- Never claim the character has been created.
- Treat established story context as canon.
- When enough information exists, return valid JSON.
- If the author is still brainstorming, respond conversationally.
...
function tryParseCharacterDraft(
  value: string,
): {
  ...
  try {
    ...
    const character =
      parsed.character ??
      (parsed as unknown as Partial<CharacterDraft>)
...
export function AIChatPanel() {
            ...
            : [
                {
                  ...
                  role: "assistant",
                  content:
                    "Let's design the character together. Tell me the idea you have so far — even if it's only a name, role, personality, or a single interesting detail.",
                },
              ],
      )
    }
  ...
  const approveCharacter =
    () => {
      if (
        ...
        setError(
          "The character needs a name before it can be created.",
        )
        return
      }
      ...
      try {
        ...
        if (!characterId) {
          throw new Error(
            "Mnemeona did not return a character ID.",
          )
        }

        updateCharacter(
          characterId,
          characterDraftToCharacter(
            characterDraft,
          ),
        )
  ...
  const renderCharacterDraft =
    () => {
      ...
      return (
        <div className="max-h-[45vh] space-y-3 overflow-y-auto rounded-lg border border-border bg-muted/30 p-3">
          <div className="flex items-center justify-between gap-2">
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                Review the profile before anything is added to your project.
              </div>
            </div>
                  ...
                  >
                    ...
                    {multiline ? (
                      <textarea
                        ...
                        className="h-16 max-h-24 w-full resize-none rounded-md border border-input bg-background px-2 py-1.5 text-sm outline-none focus:ring-1 focus:ring-ring"
                      />
                    ) : (
                      <input
                        value={value}
          ...
          {characterDraft
            ...
            <div className="rounded-md border border-border p-2 text-sm">
              ...
              {characterDraft.relationships.map(
                (
                  relationship,
                  index,
                ) => {
                  ...
                  return (
                    <div
                      key={`${relationship.targetCharacterId}-${index}`}
  ...
  return (
    <div className="flex h-full min-h-0 w-[360px] max-w-[90vw] shrink-0 flex-col overflow-hidden border-l bg-background">
      ...
      {chatMode ===
        "chat" && (
        <div className="relative shrink-0 border-b px-3 py-1.5">
          <Button
            size="sm"
            variant="ghost"
          ...
          {showContextMenu && (
            <div className="absolute left-3 top-9 z-20 w-64 rounded-md border bg-popover p-1 shadow-md">
              {contextModes.map(
                (mode) => (
                  <button
                    key={
        ...
        {messages.length ===
          ...
          chatMode ===
            ...
            <div className="py-10 text-center text-sm text-muted-foreground">
              Ask Mnemeona AI about your story, characters, scenes, or prose.
            </div>
          )}
        ...
        {createdCharacterId && (
          <div className="rounded-lg border border-border bg-muted/30 p-3 text-sm">
            ...
            <div className="mt-1 text-xs text-muted-foreground">
              The character is now part of your project and can be used by Mnemeona's character context and detection systems.
            </div>
          </div>
        )}
      ...
      <div className="shrink-0 border-t p-2">
        <div className="flex items-end gap-2">
          <textarea
            ...
            className="max-h-[120px] min-h-9 flex-1 resize-none rounded-md border border-input bg-background px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-ring"
          />
